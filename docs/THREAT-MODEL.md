...
| Host or target license-scope confusion | Require the exact 381-package `CARGO_DENY_HOST_FILTERED_GRAPH` subset of the validated 436-package graph. Bind its exact package and license semantic digests. | This inventory does not describe another host or target graph. It is not an all-target license inventory. |
...
| Evidence tampering or cherry-picking | Retain complete logs, checksums, exact commits, toolchains, seeds, status ledger, and residual risks. Retain all 13 signed version 5 mutation artifacts plus the separate exact-head CREBAIN outcome and candidate-bound hosted receipt. Bind candidate, tree, and tag in the signed four-asset release map. Reconstruct the signed tier against an independent trust root. | The CREBAIN artifact is required by the hosted workflow but is not authenticated by the version 5 signed mutation set or current finalizer. Preserve and verify it separately against the exact candidate and GitHub run. Galadriel 0.9.0 has no independent archive or DOI. GitHub availability is not permanent preservation. |
