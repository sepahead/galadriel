...
TASK_PROJECTION_SHA256 = (
    "62b57e934425d8668fa3d5e1ed5de7548bea9a693033ce10beb25fbd9aad4c86"
)
BASELINE_COMMIT = "94e2f8cc01f352d2bf899b7f656997f143a2588f"
TASK_ID = re.compile(r"T\d{3}\Z")
CLAIM_ID = re.compile(r"CLM-\d{3}\Z")
CLAIM_RISK = re.compile(r"^(T\d{3}): (CLM-\d{3}) remains NOT_CLAIMED — ")
...
def reject_duplicate_pairs(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise DispositionError("duplicate JSON key")
        result[key] = value
    return result
...
def require_text_list(
    ...
    if not isinstance(value, list) or len(value) < minimum_items:
        raise DispositionError(
            f"{context} must contain at least {minimum_items} entries"
        )
    result = [
...
def validate_tasks() -> list[dict[str, Any]]:
    ...
    if (
        source.get("original_target") != "1.0.0"
        or source.get("adapted_target") != VERSION
    ):
...
def validate_source_dispositions(
    ...
    if (
        ...
        or document["source_task_ledger_sha256"] != TASK_LEDGER_SHA256
    ):
        raise DispositionError(
            "source task-disposition state targets another release or ledger"
        )
    ...
    if (
        document["candidate_commit"] is not None
        or document["candidate_tree"] is not None
    ):
        raise DispositionError(
            "source task dispositions cannot bind a future candidate"
        )
...
def refresh_claim_limitations(
    document: dict[str, Any],
    claims: dict[str, dict[str, Any]],
) -> dict[str, Any]:
    """Refresh derived residual-risk text from the claims matrix."""

    for entry in document.get("tasks", []):
        task_id = entry.get("task_id")
        risks = entry.get("residual_risks")
        if not isinstance(task_id, str) or not isinstance(risks, list):
            continue
        for index, risk in enumerate(risks):
            if not isinstance(risk, str):
                continue
            match = CLAIM_RISK.match(risk)
            if match is None or match.group(1) != task_id:
                continue
            claim = claims.get(match.group(2))
            if claim is None or claim.get("tier") != "NOT_CLAIMED":
                continue
            risks[index] = (
                f"{task_id}: {match.group(2)} remains NOT_CLAIMED — "
                f"{claim['limitations']}"
            )
    return document
...
def _expected_exclusions(
    ...
    for index, text in enumerate(projection["procedure"]):
        if text.startswith("Have an independent reviewer reproduce"):
            expected.append(
                (f"procedure[{index}]", sha256_text(text), ("CLM-012", "CLM-016"))
            )
    for index, text in enumerate(projection["required_evidence"]):
        if text == "independent reviewer record":
            expected.append(
                (f"required_evidence[{index}]", sha256_text(text), ("CLM-012",))
            )
    return expected
...
def _validate_projection(
    ...
    completion = require_text(
        projection["completion_rule"], f"{task_id}/completion rule", minimum=80
    )
    ...
    if not isinstance(review, dict) or tuple(review) != LENSES:
        raise DispositionError(
            f"{task_id}: source review must contain ordered L01--L20"
        )
    for lens in LENSES:
        value = review[lens]
        require_keys(
            value, {"question", "finding", "evidence", "status"}, f"{task_id}/{lens}"
        )
        ...
        if (
            ...
            raise DispositionError(
                f"{task_id}/{lens}: source plan contains a fabricated result"
            )
    ...
    if (
        ...
        or entry["claim_removal_links"] != expected_claims
    ):
    ...
    for claim_id in expected_claims:
        if claims.get(claim_id, {}).get("tier") != "NOT_CLAIMED":
            raise DispositionError(
                f"{task_id}: task exclusion points to an affirmative claim"
            )

    accepted = require_text_list(
        entry["accepted_cases"],
        f"{task_id}/accepted cases",
        minimum_items=2,
        minimum_length=80,
    )
    if not all(
        task_id in value and projection["title"] in value for value in accepted[:1]
    ):
    ...
    if not isinstance(rejected, list) or len(rejected) != len(counterfactuals):
        raise DispositionError(
            f"{task_id}: rejected cases do not cover all counterfactuals"
        )
    for index, (item, counterfactual) in enumerate(
        zip(rejected, counterfactuals, strict=True), 1
    ):
        require_keys(
            item,
            {"id", "source_sha256", "case", "rejection_rule"},
            f"{task_id}/rejected case {index}",
        )
        if (
            item["id"] != f"GLD-090-{task_id}-REJ-{index:02d}"
            or item["case"] != counterfactual
        ):
            raise DispositionError(f"{task_id}: rejected case sequence changed")
        if (
            item["source_sha256"] != sha256_text(counterfactual)
            or task_id not in item["rejection_rule"]
        ):
            raise DispositionError(
                f"{task_id}: rejected case lacks exact source binding"
            )
    if entry["evidence_types"] != projection["required_evidence"]:
        raise DispositionError(
            f"{task_id}: required evidence differs from source ledger"
        )
    failures = require_text_list(
        entry["failure_record_requirements"],
        f"{task_id}/failure records",
        minimum_items=2,
        minimum_length=70,
    )
    ...
    for actual, (path, source_digest, links) in zip(
        exclusions, expected_exclusions, strict=True
    ):
        require_keys(
            actual,
            {"source_path", "source_sha256", "claim_removal_links", "reason"},
            f"{task_id}/source exclusion",
        )
        if (
    ...
    for risk in risks:
        ...
        if claim is None or claim.get("tier") != "NOT_CLAIMED":
            raise DispositionError(
                f"{task_id}: residual risk references an invalid unclaimed claim"
            )
        expected = f"{task_id}: {claim_id} remains NOT_CLAIMED — {claim['limitations']}"
        if risk != expected:
            raise DispositionError(
                f"{task_id}: residual risk differs from {claim_id} limitations"
            )
...
def validate_plan(
    ...
    if (
        ...
        raise DispositionError(
            "task closure plan targets another release or handoff ledger"
        )
    ...
    boundary = require_text(
        document["closure_boundary"], "closure boundary", minimum=80
    )
    for required in ("post-commit", "outside", "does not establish completion"):
        if required not in boundary:
            raise DispositionError(
                f"closure boundary omits required phrase: {required}"
            )
    ...
    for lens in LENSES:
        ...
        require_text(
            catalog[lens]["question"], f"lens catalog/{lens}/question", minimum=40
        )
    ...
    if (
        document["source_projection_sha256"] != combined
        or combined != TASK_PROJECTION_SHA256
    ):
        raise DispositionError(
            "source task projection differs from the immutable handoff ledger"
        )
    return document
...
def verify() -> None:
    ...
    print(
        "byte-bound task-closure plan verified; 107 tasks remain post-commit and 9 are NOT_CLAIMED"
    )


def generate() -> None:
    claims = validate_claims()
    document = refresh_claim_limitations(load_json(PLAN_PATH), claims)
    validate_plan(claims=claims, document=document)
    PLAN_PATH.write_bytes(canonical_bytes(document))
    print("task-closure plan claim limitations refreshed")
