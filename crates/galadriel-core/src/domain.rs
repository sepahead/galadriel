...
macro_rules! bounded_numeric_identifier {
    ($name:ident, $kind:literal, $documentation:literal) => {
        ...
        impl $name {
            ...
            /// # Errors
            ///
            /// Returns [`DomainError::ZeroIdentifier`] for zero and
...
bounded_numeric_identifier!(
    TrackId,
    "track_id",
    "A nonzero track identity with an exact cross-language JSON representation."
);
bounded_numeric_identifier!(
    ProjectionFrameId,
    "projection_frame_id",
...
mod tests {
    ...
    #[test]
    fn numeric_identities_reject_zero() {
        assert_eq!(
            TrackId::new(0),
            Err(DomainError::ZeroIdentifier { kind: "track_id" })
        );
    }
