...
const TRIAL_SCHEMA: &str = "galadriel.evidence.trial.v1";
...
const MAX_METRICS_PER_CONDITION: usize = 5;
...
impl EvidenceConfig {
    fn validate(&self) -> AppResult<()> {
        ...
        (self.frames as u64)
            ...
            .ok_or_else(|| "frames * dt_ms overflows u64".to_string())?;
...
fn synthetic_specs(config: &EvidenceConfig) -> AppResult<Vec<(ExperimentSpec, bool)>> {
...
fn deterministic_missing(seed: u64, frame: usize, modality: Modality, probability: f64) -> bool {
    ...
    let bits = mix64(seed ^ (frame as u64).wrapping_mul(0xd1b5_4a32_d192_ed03) ^ modality as u64);
...
impl TraceAccumulator {
    ...
    fn finish(self, context: FinishContext<'_, '_>) -> TrialRecord {
        let FinishContext {
            meta,
            detector,
            track_id,
        ...
        let (pre_onset_alert, first_post_onset_delay_frames, attribution_correct) =
            if let Some(onset) = meta.truth.onset_frame {
                ...
                let delay = (!pre_onset)
                    ...
                    .flatten();
                let attribution = (!pre_onset)
                    .then(|| {
                        post.map(|episode| {
                            let mut observed = episode.channels.clone();
                            observed.sort_by_key(|modality| *modality as u8);
                            observed.dedup();
                            let mut expected = meta.truth.channels.clone();
                            expected.sort_by_key(|modality| *modality as u8);
                            expected.dedup();
                            episode.classification == "attributed_inconsistency"
                                && observed == expected
                        })
                    })
                    .flatten();
                (Some(pre_onset), delay, attribution)
            } else {
                (None, None, None)
            };
...
fn monitoring_start_frame(config: &EvidenceConfig, detector: DetectorId) -> usize {
    let required_samples = match detector {
        DetectorId::NisBaseline => config.detector.min_samples,
        DetectorId::DefaultCorrelationFusion => config
            .detector
            .min_samples
            .max(config.correlation.min_samples),
    };
...
fn evaluate_track(
    ...
    while frame_start < stream.len() {
        ...
        for observation in &stream[frame_start..frame_end] {
            mirror.ingest(observation).map_err(|error| {
                format!(
                    ...
                    Ok(Some(projection)) => {
                        ...
                        for (axis, channels) in projection.axes.iter().enumerate() {
                            ...
                            match correlation::analyze(channels, &adjusted) {
                                Ok(report) => {
                                    correlations.push(AxisCorrelationReport { axis, report })
                                }
                                Err(_) => {
...
fn run_synthetic(config: &EvidenceConfig) -> AppResult<Vec<TrialRecord>> {
...
fn preflight_recorded_work(
    ...
    while track_start < observations.len() {
        ...
        let assessments = frames.div_ceil(config.assessment_step);
        ...
        let track_correlation_work = assessments
            .checked_mul(3)
            .and_then(|work| work.checked_mul(modality_pairs))
            .and_then(|work| work.checked_mul(config.correlation.window.min(frames)))
...
fn run_recorded(
    ...
    let read_limit = u64::try_from(maximum_bytes)
        .map_err(|_| "recorded parser byte limit does not fit u64".to_string())?
        .checked_add(1)
        .ok_or_else(|| "recorded parser byte limit overflowed".to_string())?;
    let file = File::open(fixture_path)
        .map_err(|error| format!("open recorded fixture {}: {error}", fixture_path.display()))?;
    let mut fixture_bytes = Vec::new();
    file.take(read_limit)
        .read_to_end(&mut fixture_bytes)
        .map_err(|error| format!("read recorded fixture {}: {error}", fixture_path.display()))?;
    if fixture_bytes.len() > maximum_bytes {
        return Err(format!(
            "recorded fixture exceeds the parser maximum of {maximum_bytes} bytes"
        ));
    }
    let actual_hash = sha256_bytes(&fixture_bytes);
    if actual_hash != config.recorded_fixture.sha256.to_ascii_lowercase() {
        return Err(format!(
            "recorded fixture SHA-256 mismatch for {}: configured {}, actual {actual_hash}",
            fixture_path.display(),
            config.recorded_fixture.sha256
        ));
    }
    let fixture_text = std::str::from_utf8(&fixture_bytes).map_err(|error| {
        format!(
            "recorded fixture {} is not UTF-8: {error}",
            fixture_path.display()
        )
    })?;
    let mut observations = galadriel_ncp::parse_jsonl(fixture_text)
        .map_err(|error| format!("parse recorded fixture {}: {error}", fixture_path.display()))?;
    if observations.is_empty() {
        return Err("recorded fixture contains no observations".into());
    }
    ...
    while track_start < observations.len() {
        ...
        records.extend(evaluate_track(stream, &modalities, &meta)?);
        trial_index += 1;
        track_start = track_end;
    }
    all_reasons.sort();
    all_reasons.dedup();
    let resolved_path = fixture_path
        .canonicalize()
        .unwrap_or_else(|_| fixture_path.to_path_buf());
    let info = RecordedRunInfo {
        configured_path: config.recorded_fixture.path.clone(),
...
impl MetricKind {
    ...
    fn unit(self) -> &'static str {
        match self {
            ...
            | Self::ConditionalAttributionAccuracy => "probability",
        }
    }
    ...
    fn method(self) -> &'static str {
        match self {
            ...
            Self::ConditionalAttributionAccuracy => {
                "exact first-alert channel attribution among detected tracks without pre-onset alert"
            }
            Self::AbstentionFraction => {
...
fn binomial_counts(records: &[&TrialRecord], kind: MetricKind) -> Option<(usize, usize)> {
    let (successes, trials) = match kind {
        ...
        | MetricKind::Arl0Assessments
        | MetricKind::AbstentionFraction
        | MetricKind::ConditionalDelayFrames => return None,
    };
...
fn estimate_metric(
    ...
    if kind == MetricKind::ConditionalDelayFrames
        && eligible_tracks < config.min_metric_eligible_tracks
    {
        return MetricEstimate {
            ...
            estimator: kind.method().into(),
            interval: format!(
                "not estimable: {eligible_tracks} detected eligible tracks is below configured minimum {}",
    ...
    let (ci95, ci_status, interval) = if kind.is_track_binomial() {
        ...
        (
            Some(interval_envelope([lower, upper], usable_bootstrap_ci)),
            "estimated".into(),
            format!(
                "95% Wilson score interval for a track-level binomial proportion; whole-track bootstrap envelope used only when at least 80% of replicates are usable ({bootstrap_usable} usable of {} requested)",
...
fn summarize_condition(records: &[&TrialRecord], config: &EvidenceConfig) -> ConditionSummary {
    let first = records[0];
...
fn build_summary(
    ...
    EvidenceSummary {
        schema: "galadriel.evidence.summary.v1".into(),
        ...
        interval_method: format!(
            "Boundary-safe 95% intervals: Wilson for track proportions, Garwood Poisson count-rate intervals for alert episodes, and distribution-free bounded-track Hoeffding intervals for ARL0 and post-warm-up abstention, conservatively enveloped by whole-track bootstrap when >=80% of {} requested replicates are usable; whole-track percentile bootstrap for delay",
            config.bootstrap_resamples
        ),
...
fn require_publication_clean(git: &GitProvenance, allow_dirty: bool) -> AppResult<()> {
...
fn build_manifest(input: ManifestBuild<'_>) -> AppResult<EvidenceManifest> {
    ...
    let synthetic_tracks = records
        ...
        .count();
    Ok(EvidenceManifest {
        schema: "galadriel.evidence.manifest.v1".into(),
        study_id: config.study_id.clone(),
        base_seed_hex: format!("0x{:016x}", config.base_seed),
        git,
        toolchain: ToolchainProvenance {
...
fn render_report(summary: &EvidenceSummary, manifest: &EvidenceManifest) -> String {
    ...
    report.push_str("Only holdout rows below support reported results. Calibration tracks are retained in `summary.json` as separate diagnostics and are never pooled. Proportions use Wilson intervals, alert-episode rates use labeled Garwood Poisson intervals, and other intervals use whole-track bootstrap; eligible bootstrap envelopes never replace the boundary-safe analytic interval.\n\n");
    ...
    report.push_str("| condition | detector | exposure h | false alerts/hour | mission P(any) | restricted ARL0 | ARL0 censored | abstention | provenance P(any alert) | pre-onset P | conditional detection | conditional delay | attribution |\n");
    report.push_str("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n");
    ...
    report.push_str("\n## Recorded fixture\n\n");
    report.push_str(&format!(
        "Status: `{}`; {} observations across {} track(s), {} ms total observed duration, {} observations with a consistency projection.\n\n",
...
fn write_artifacts(
    ...
    write_bytes(&config_path, &json_pretty(config)?)?;
...
fn resolve_fixture_path(config_path: &Path, configured: &str) -> AppResult<PathBuf> {
    ...
    if !path.is_file() {
        return Err(format!(
            "recorded fixture is not a file: {}",
            path.display()
        ));
    }
    Ok(path)
...
fn run() -> AppResult<()> {
    ...
    let config: EvidenceConfig = serde_json::from_slice(&config_bytes)
        .map_err(|error| format!("parse config {}: {error}", config_path.display()))?;
    config.validate()?;
    let fixture_path = resolve_fixture_path(&config_path, &config.recorded_fixture.path)?;
    let (workspace_root, initial_git) = git_snapshot(&config_path)?;
    require_publication_clean(&initial_git, arguments.allow_dirty)?;

    let mut records = run_synthetic(&config)?;
    let (recorded_records, mut recorded_info) = run_recorded(&config, &fixture_path)?;
    ...
    let canonical_config = json_pretty(&config)?;
    ...
    require_publication_clean(&final_git, arguments.allow_dirty)?;
    let manifest = build_manifest(ManifestBuild {
        ...
        git: final_git,
        dirty_override_used: arguments.allow_dirty,
    })?;
    write_artifacts(
...
mod tests {
    ...
    fn tiny_config() -> EvidenceConfig {
        EvidenceConfig {
            schema_version: ARTIFACT_SCHEMA_VERSION,
            study_id: "test_evidence".into(),
            base_seed: 0x5eed_cafe_1234_5678,
    ...
    fn fake_manifest(config: &EvidenceConfig, record_count: usize) -> EvidenceManifest {
        EvidenceManifest {
            schema: "galadriel.evidence.manifest.v1".into(),
            study_id: config.study_id.clone(),
            base_seed_hex: format!("0x{:016x}", config.base_seed),
    ...
    fn synthetic_records_are_deterministic_and_partitions_are_disjoint() {
        ...
        config.validate().expect("tiny config should validate");
        ...
        for record in &first {
            assert_eq!(
                record.seed_hex,
                record.seed.map(|seed| format!("0x{seed:016x}"))
            );
    ...
    fn float_arms_use_exact_ids_and_normalize_signed_zero() {
        ...
        config.validate().expect("close float arms should validate");
        ...
        config.autocorrelation_phis = vec![0.0, -0.0, 0.6];
        let error = config
            .validate()
    ...
    fn validation_bounds_study_shape_and_repeated_work() {
        ...
        assert!(run_synthetic(&config)
            .expect_err("direct synthetic execution must reject overflowing counts")
            .contains("calibration + holdout"));
        ...
        let mut recorded_config = tiny_config();
        recorded_config.assessment_step = 1;
        recorded_config.correlation.window = 65_536;
        recorded_config
            .validate()
        ...
        let recorded = (0..8_000_u64)
            ...
            .collect::<Vec<_>>();
        assert!(preflight_recorded_work(&recorded, &recorded_config)
    ...
    fn dirty_publication_requires_an_explicit_development_override() {
        ...
        assert!(require_publication_clean(&git, false).is_err());
        require_publication_clean(&git, true).expect("development override should be explicit");
    ...
    fn published_config_is_valid_and_has_declared_long_holdout_exposure() {
        ...
        let config: EvidenceConfig =
            serde_json::from_slice(&bytes).expect("published config should parse");
        config.validate().expect("published config should validate");
    ...
    fn recorded_fixture_is_explicitly_not_estimable() {
        let config = tiny_config();
        let (records, info) =
            run_recorded(&config, &fixture_path()).expect("recorded fixture should replay");
