...
fn replay_stdout(max_report_tracks: &str) -> String {
...
fn replay_reports_suppressed_histories_and_exact_limit_boundaries() {
    let suppressed = replay_stdout("1");
    ...
    assert!(reported_track_ids(&suppressed, "│  MI       · track ").is_empty());
    ...
    let all_visible = replay_stdout("3");
    assert_eq!(
    ...
    assert!(reported_track_ids(&all_visible, "│  MI       · track ").is_empty());
